...
function bachGiftCards(nodeapiurl, $resource, toastr, $http, OrderCloudSDK) {
    ...
    function _update(req) {
        ...
        return GiftCards().update({
            id: req.body.id
        }, req.body).$promise;
    }

    function _delete(req) {
        return GiftCards().delete({
            id: req.id
        }).$promise;
    }

    function _list(req) {
        return GiftCards().list(req).$promise
    ...
    function _purchase(req) {
        return $http.post(nodeapiurl + '/giftcards/purchase/' + req.orderid, {}, {
            headers: {
    ...
    function _release(req) {
        return $http.post(nodeapiurl + '/giftcards/release/' + req.orderid, {}, {
            headers: {
                'oc-token': OrderCloudSDK.GetToken()
            }
        });
